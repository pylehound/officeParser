...
function parseAnnotationMetadata(buffer, annotationsText) {
    try {
        ...
        if (!wdEntry)
            return null;
        const wdBuf = Buffer.from(wdEntry.content);
        ...
        const tblEntry = cfb_1.default.find(cfb, tblName);
        if (!tblEntry)
            return null;
        const tblBuf = Buffer.from(tblEntry.content);
        ...
        if (lcbPlcfandRef < 28 || lcbPlcfandTxt < 8)
            return null;
        // PlcfandRef: (nAtn+1)*4 bytes of CPs + nAtn*20 bytes of ATRD
        const nAtn = (lcbPlcfandRef - 4) / 24;
        if (!Number.isInteger(nAtn) || nAtn < 1)
            return null;
        if (lcbPlcfandTxt !== (nAtn + 1) * 4)
            return null;
        // Validate buffer bounds
        if (fcPlcfandRef < 0 || fcPlcfandRef + lcbPlcfandRef > tblBuf.length)
            return null;
        if (fcPlcfandTxt < 0 || fcPlcfandTxt + lcbPlcfandTxt > tblBuf.length)
            return null;
        ...
        for (let i = 0; i < nAtn; i++) {
            ...
            if (relStart < 0 || relEnd > annotationsText.length || relStart >= relEnd)
                return null;
    ...
    if (annotationsText.trim()) {
        ...
        if (perAnnotation) {
            for (const { author, text } of perAnnotation) {
                if (text) {
                    const commentBlock = { type: 'comment', text, author };
                    blocks.push(commentBlock);
                }
            }
        }
        else {
            const commentBlock = { type: 'comment', text: annotationsText.trim() };
            blocks.push(commentBlock);
        }
    }
